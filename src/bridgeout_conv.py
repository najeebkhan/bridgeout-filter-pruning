...
@weak_module
class BridgeoutConvLayer(_ConvNd):
    ...
    * :attr:`groups` controls the connections between inputs and outputs.
    ...
    """
    def __init__(self, in_channels, out_channels, kernel_size, p, q, target_fraction=1.0, stride=1,
                 padding=0, dilation=1, groups=1,  bias=True, unit_test_mode=False):
        if unit_test_mode:
            seed = 979
            torch.manual_seed(seed)
            torch.cuda.manual_seed(seed)
        ...
        super(BridgeoutConvLayer, self).__init__(
...
if __name__ == '__main__':
    c = BridgeoutConvLayer(1,1,3, p=0.5, q=None).double()
    print(c)
    print()
    x = torch.ones(2, 1, 6, 6, dtype=torch.double,requires_grad=True)
    for i in range(3):
        y = c(x)
        y.backward(torch.ones(y.size()).double())
        [print('p, p.grad', n, p.grad) for n, p in c.named_parameters()]
        print(y)
        print('*'*80)
        c.zero_grad()
