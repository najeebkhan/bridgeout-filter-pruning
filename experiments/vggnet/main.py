...
from vggnet import VGG, conv_init
...
def getArguments():
    ...
    parser.add_argument('--seed', default=43, type=int, help='training seed')
...
def getNetwork(args):
    ...
    if (args.net_type == 'lenet'):
        ...
        net = VGG(args.depth, num_classes, regularizer)
        file_name = 'vgg-'+str(args.depth)+'_'+args.reg_type+'_'+args.dataset+'_p_'+str(args.dropout)+'_q_'+str(args.act_norm)+'_target_'+str(args.tar_frac)
...
class Trainer:
    ...
    def build_net(self, config):
        ...
        self.model_pth='/home/najeeb/dnn_training/bo_compression/exp5/results/'+self.file_name
        # self.model_pth='/mnt/DATA/dnn_training/bo_compression/exp5/results/'+self.file_name
        ...
        for name, _ in self.net.named_parameters():
            if 'weight' in name and 'linear' not in name and 'bn' not in name:
                ...
                print('excluded from pruning', name)
